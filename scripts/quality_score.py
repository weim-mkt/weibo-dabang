#!/usr/bin/env python3
"""
Quality Scoring System for the Weibo Dabang JMR R&R

Calculates objective quality scores (0-100) for Quarto slide decks and R
analysis scripts. Beamer scoring was dropped because the paper lives in the
Overleaf submodule and compiles there, not locally.

Usage:
    python scripts/quality_score.py Quarto/InternalUpdate.qmd
    python scripts/quality_score.py Quarto/InternalUpdate.qmd --summary
    python scripts/quality_score.py Quarto/*.qmd
    python scripts/quality_score.py code/03_did_main.R
"""
...
class QualityScorer:
    ...
    def score_quarto(self) -> Dict:
        ...
        # The bib lives in the overleaf-weibo-dabang submodule; fall back gracefully if missing.
        bib_file = self.filepath.parent.parent / 'overleaf-weibo-dabang' / 'references2.bib'
...
Examples:
  ...
  python scripts/quality_score.py Quarto/InternalUpdate.qmd --verbose

Quality Thresholds:
  80/100 = Commit threshold (blocks if below)
  90/100 = Submit-ready threshold (warning if below)
  95/100 = Excellence (aspirational)

Exit Codes:
  0 = Score >= 80 (commit allowed)
  1 = Score < 80 (commit blocked)
  2 = Auto-fail (render/syntax error)
        """
    )
    ...
    for filepath in args.filepaths:
        ...
        try:
            ...
            if filepath.suffix == '.qmd':
                ...
                print(f"Note: .tex scoring removed in this project (paper lives in Overleaf submodule). "
                      f"Skipping {filepath}.")
                continue
            else:
